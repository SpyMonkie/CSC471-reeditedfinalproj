...
public:
	...
	void initGeom(const std::string& resourceDirectory)
	{
		...
		vector<tinyobj::shape_t> TOshapes3;
 		rc = tinyobj::LoadObj(TOshapes3, objMaterials, errStr, (resourceDirectory + "/dog.obj").c_str());
	...
	void render() {
		...
		Model->loadIdentity();
			...
			Model->rotate(dogTheta, vec3(1, 0, 0));
		...
		//use helper function that uses glm to create some transform matrices
		setModel(prog, vec3(1.7, -1.7, 0), 2.1, 0, 0.5);
		bunny->draw(prog);
		...
		sTheta = sin(2*glfwGetTime());
		dogTheta = sin(1.5 * glfwGetTime()) / 2;
